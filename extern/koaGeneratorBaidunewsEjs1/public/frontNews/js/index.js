...
$(document).ready(function() {
    $(function() {
        ...
        window.onscroll = function() {
            if ((table== 'news')&&($(".table-jingxuan .news-item").length > 0))
            {
                if (loadImgOrNotViaMysql(tableToclassName['news'])) {
                    requestDataViaMysql();
                }
            }
            else if((table!== 'news')&&($(".table-baijia .news-item").length > 0)){
                    requestDataViaMysql();
            }
        }
        ...
        for (var i = 0; i < 23; i++) {
            (function(i) {
                $(".mainMenu>li:eq(" + i + ")").click(function() {
                    ...
                    if ($(".list-item-table .news-item").length !== 0) {
                        $(".list-item-table .news-item").remove();
                    }
                })
            })(i);
        }
        ...
        success: function(data) {
            ...
            data.forEach(function(element, index) { //用each失败
                ...
                if (imgSrc.length == 0) { //0张图片
                    ...
                    dataImg.newsFrom[0][0].srcTime = moment(element['srcTime']).format('YYYY-MM-DD');
                ...
                } else if (imgSrc.length == 1) //1张图片
                {
                    ...
                    dataImg.newsFrom[1][0].srcTime = moment(element['srcTime']).format('YYYY-MM-DD');
                    ...
                    dataImg.newsFrom[2][0].srcTime = moment(element['srcTime']).format('YYYY-MM-DD');
                    dataImg.newsFrom[2][0].srcIcon0 = element['srcIcon0'];
